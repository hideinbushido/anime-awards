...
export default function NomineesClient({
  ...
  // Bloque le public si le vote n'est pas encore ouvert
  if (authChecked && !isAdmin && eventStatus !== 'voting_open') {
    ...
    return (
      <div>
        <div className="relative flex items-center gap-3 mb-8 rounded-2xl overflow-hidden"
          style={fondUrl ? { padding: '20px 20px', backgroundImage: `url('${fondUrl}')`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}
        >
          ...
          <div className="relative">
            <h2 className="text-2xl font-black text-white">{locale === 'fr' ? category.titleFr : category.titleEn}</h2>
          </div>
        </div>
        <div className="flex flex-col items-center justify-center py-24 gap-6">
          ...
          <p className="text-center max-w-xs" style={{ color: '#9a8870' }}>
            Les nominés seront révélés lors de l&apos;ouverture du vote. Restez connectés !
          </p>
        </div>
      </div>
    );
